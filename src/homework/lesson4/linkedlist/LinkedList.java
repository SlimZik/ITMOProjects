...
public class LinkedList<T> implements List<T>, Stack<T> {
    ...
    @Override
    public T get(int i) throws MyExeptions{                                                                                               // Взятие элемента по индексу
        if (i < 0 || i > size) throw new MyExeptions ("Incorrect index Exception");
        if (head == null)
            return null;
    ...
    @Override
    public Iterator<T> iterator() throws MyExeptions{
